...
import pytorch_nndct.nn.modules.functional 
...
class ConvBlock(nn.Module):
    def __init__(self, in_channel, out_channel, strides=1):
        ...
        self.nndct_add = pytorch_nndct.nn.modules.functional.Add()
...
class UNet(nn.Module):
    def __init__(self, block=ConvBlock,dim=32):
        ...
        self.nndct_add = pytorch_nndct.nn.modules.functional.Add()
        self.nndct_cat1 = pytorch_nndct.nn.modules.functional.Cat()
        self.nndct_cat2 = pytorch_nndct.nn.modules.functional.Cat()
        self.nndct_cat3 = pytorch_nndct.nn.modules.functional.Cat()
        self.nndct_cat4 = pytorch_nndct.nn.modules.functional.Cat()
    ...
    def forward(self, x):
        ...
        up6 = self.nndct_cat1([up6, conv4], 1)
        conv6 = self.ConvBlock6(up6)

        up7 = self.upv7(conv6)
        #up7 = torch.cat([up7, conv3], 1)
        up7 = self.nndct_cat2([up7, conv3], 1)
        conv7 = self.ConvBlock7(up7)

        up8 = self.upv8(conv7)
        #up8 = torch.cat([up8, conv2], 1)
        up8 = self.nndct_cat3([up8, conv2], 1)
        conv8 = self.ConvBlock8(up8)

        up9 = self.upv9(conv8)
        #up9 = torch.cat([up9, conv1], 1)
        up9 = self.nndct_cat4([up9, conv1], 1)
        conv9 = self.ConvBlock9(up9)

        conv10 = self.conv10(conv9)
        #out = x + conv10
        out = self.nndct_add(x,conv10)

        out = self.dequant_stub(out)
        return out
